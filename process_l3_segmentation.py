"""
L3 / L1-L5 Slice Segmentation Processor
=========================================
...
Usage:
    python process_l3_segmentation.py --input <csv_file> [--bbox_ant_mm 70] [--bbox_lr_mm 100] [--output ./l3_output]
...
Also appended to input CSV (saved as <output>/results_summary.csv):
    l3_<n>_area_cm2           Cross-sectional area of each soft-tissue label on L3 slice
    l3_<n>_mean_hu            Mean HU of each soft-tissue label on L3 slice
    l3_<n>_min_hu             Min HU of each soft-tissue label on L3 slice
    l3_<n>_max_hu             Max HU of each soft-tissue label on L3 slice
    l3_<n>_median_hu          Median HU of each soft-tissue label on L3 slice
    ...
    vol_<n>_volume_cm3        Volume of each label across L1-L5
    vol_<n>_mean_hu           Mean HU of each label across L1-L5

where <stem> is the original NIfTI filename without extension,
and <n> is one of: subq_fat, imat, muscle.
"""
...
L3_VERTEBRA_TOTAL_LABEL = 29          # total label for L3 vertebral body
...
def _sc_center_at(total_data, idx, axis):
    slc  = extract_slice_2d(total_data, idx, axis).astype(np.int32)
    mask = slc == SPINAL_CORD_LABEL
    if not mask.any():
        raise ValueError(f"SC label absent at slice {idx}")
...
def find_spinal_cord_center_with_fallback(total_data, slice_idx, axial_axis,
    ...
    for offset in offsets:
        ...
        if not (0 <= idx < total_n):
            continue
        try:
            _, _, center = _sc_center_at(total_data, idx, axial_axis)
            if offset != 0:
            ...
            continue
    raise ValueError(
        f"Spinal cord (label {SPINAL_CORD_LABEL}) not found within "
...
def measure_vertebra_l3_slice(ct_slice, total_slice, pixdim, axial_axis):
    ...
    L3 vertebral body (total label 29) on the L3 axial slice.
    ...
    if n_px > 0:
        ...
        print(f"  WARNING: L3 vertebra label ({L3_VERTEBRA_TOTAL_LABEL}) absent "
...
def process_volume_l1_l5(ct_data, total_data, tissue_data,
                          ...
                          orientation_6, voxel_size):
    """
    Process every axial slice in the L1-L5 span.
    Psoas direction locked once from first detectable slice; SC re-detected per slice.
    ...
    for probe_idx in range(slc_min, slc_max + 1):
        try:
            probe_sc, _ = find_spinal_cord_center_with_fallback(
                total_data, probe_idx, axial_axis)
...
def process_row(row, bbox_ant_mm, bbox_lr_mm, base_output_dir):
    ...
    sc_ctr, sc_src = find_spinal_cord_center_with_fallback(
        total_data, slice_idx, axial_axis)
...
def main():
    ...
    parser.add_argument("--output",  default="./l3_output",
    ...
    for i, row in df.iterrows():
        try:
            stats = process_row(row, args.bbox_ant_mm, args.bbox_lr_mm, args.output)
    ...
    out_csv = os.path.join(args.output, "results_summary.csv")
